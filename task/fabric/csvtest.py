...
#node1---5002
    ...
    media92n=getnumber('10.30.1.72','media5002')
...
def getSpaceOne(dict):
    try:
        ...
        if(dict['username'] in ['acd','cti','smp']):
            result2=conn.run('/home/mysql/bin/mysql -ucincc -pcinCC1234 -e "show slave status\G"|grep -A 1 Slave_IO_Running')
            number2= result2.stdout.strip()
            dict['mySQL']=number2
        #判断ms同步情况
        if(dict['username'] =='ms'):
            result3=conn.run('ls -t /home/ms/cin/sync/sync*|head -1|cut -d \'/\' -f 6 && cat /home/ms/cin/sync/mergesync.pos')
            ...
            if(dict['ip'] in ['10.30.1.101','10.30.1.102','10.30.1.103','10.30.1.104','10.30.1.105','10.30.1.106','10.30.1.107','10.30.1.108']):
                try:
                    ...
                    result4 = conn1.run('ls -t /home/ms1/cin/sync/sync*|head -1|cut -d \'/\' -f 6 && cat /home/ms1/cin/sync/mergesync.pos')
